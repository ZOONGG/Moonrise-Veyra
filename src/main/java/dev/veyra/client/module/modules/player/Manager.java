...
import dev.veyra.client.utils.player.ItemUtils;
import dev.veyra.client.utils.player.PotionUtils;
...
public class Manager extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (!this.stopwatch.hasReached(this.nextClick) || this.chestTicks < 10) return;
        ...
        for (int i = 0; i < 40; i++) {
            ...
            if (item instanceof ItemPotion) {
                Integer effectId = firstPotionEffectId(stack);
                ...
                if (potion == -1) {
                    ...
                    Integer currentEffectId = firstPotionEffectId(currentStack);
                    ...
                    for (final PotionEffect e : mc.thePlayer.getActivePotionEffects()) {
                        if (e.getPotionID() == currentEffectId && e.getDuration() > 0) {
                            foundCurrent = true;
                            break;
                        }
                    }
                    ...
                    for (final PotionEffect e : mc.thePlayer.getActivePotionEffects()) {
                        if (e.getPotionID() == effectId && e.getDuration() > 0) {
                            found = true;
                            break;
                        }
                    }

                    if ((PotionUtils.potionRanking(effectId) > PotionUtils.potionRanking(currentEffectId) || foundCurrent) && !found) {
                        potion = i;
                    }
                }
            }
        ...
        for (int i = 0; i < 40; i++) {
            ...
            if (item instanceof ItemSword && i != sword) this.throwItem(i);
            if (item instanceof ItemPickaxe && i != pickaxe) this.throwItem(i);
            if (item instanceof ItemAxe && i != axe) this.throwItem(i);
            if (item instanceof ItemSpade && i != shovel) this.throwItem(i);
        }
        ...
        if (sword != -1 && sword != (int) swordSlot.getInput() - 1) {
            this.moveItem(sword, (int) swordSlot.getInput() - 1);
        }

        if (block != -1 && block != (int) blockSlot.getInput() - 1) {
            this.moveItem(block, (int) blockSlot.getInput() - 1);
        }
        ...
        if (pickaxe != -1 && pickaxe != pickaxeSlotInt - 1) {
            if (!keepTools.isToggled()) {
                ...
                this.moveItem(pickaxe, pickaxeSlotInt - 1);
            }
        }
        ...
        if (axe != -1 && axe != axeSlotInt - 1) {
            if (!keepTools.isToggled()) {
                ...
                this.moveItem(axe, axeSlotInt - 1);
            }
        }
        ...
        if (shovel != -1 && shovel != shovelSlotInt - 1) {
            if (!keepTools.isToggled()) {
                ...
                this.moveItem(shovel, shovelSlotInt - 1);
            }
        }

        if (potion != -1 && potion != potionSlotInt - 1) {
            this.moveItem(potion, potionSlotInt - 1);
        }

        if (food != -1 && food != foodSlotInt - 1) {
            this.moveItem(food, foodSlotInt - 1);
        }
    ...
    private void moveItem(final int slot, final int destination) {
        if ((!this.moved || this.nextClick <= 0)) {
            ...
            mc.playerController.windowClick(mc.thePlayer.inventoryContainer.windowId, this.slot(slot), destination, 2, mc.thePlayer);
    ...
    private Integer firstPotionEffectId(final ItemStack stack) {
        if (!(stack.getItem() instanceof ItemPotion potion)) return null;
        java.util.List<PotionEffect> effects = potion.getEffects(stack);
        return effects == null || effects.isEmpty() ? null : effects.get(0).getPotionID();
    }
    ...
    public static void getSlots(int int1, int int2) {
        ...
        for (int i = 1; i <= 9; i++) {
            if (i != int1 && i != int2 && index < inventorySlots.length) {
