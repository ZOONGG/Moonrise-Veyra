...
public class CategoryComponent {
   ...
   public CategoryComponent(Module.ModuleCategory category) {
      ...
      for(Iterator<Module> var3 = Veyra.moduleManager.getModulesInCategory(this.categoryName).iterator(); var3.hasNext(); tY += 16) {
   ...
   public void setX(int n) {
      this.x = n;
      if(Veyra.clientConfig != null){
         Veyra.clientConfig.saveConfig();
      }
   }
   ...
   /**
    * Moves a floating panel without writing the client config on every mouse
    * movement. ClickGui persists the final position when dragging ends.
    */
   ...
   public void rf(FontRenderer renderer) {
      ...
      renderer.drawStringWithShadow(this.n4m ? this.pvp : this.categoryName.name(), (float)(this.x + 2), (float)(this.y + 4), Theme.getTextPrimaryColor().getRGB());
      if (!this.n4m) {
         GL11.glPushMatrix();
         renderer.drawStringWithShadow(this.categoryOpened ? "-" : "+", (float)(this.x + marginX), (float)((double)this.y + marginY), Theme.getMainColor().getRGB());
         GL11.glPopMatrix();
