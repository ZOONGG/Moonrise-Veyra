...
public class ModuleComponent implements Component {
   ...
   public static void g(int h) {
      float a = (float)(h >> 24 & 255) / 255.0F;
      float r = (float)(h >> 16 & 255) / 255.0F;
      float g = (float)(h >> 8 & 255) / 255.0F;
      float b = (float)(h & 255) / 255.0F;
   ...
   public void draw() {
      int topColor = this.mod.isEnabled() ? Theme.getMainColor().darker().getRGB() : Theme.getSurfaceHoverColor().getRGB();
      int bottomColor = this.mod.isEnabled() ? Theme.getMainColor().getRGB() : Theme.getSurfaceColor().getRGB();
      v((float)this.category.getX(), (float)(this.category.getY() + this.o), (float)(this.category.getX() + this.category.getWidth()), (float)(this.category.getY() + 15 + this.o), bottomColor, topColor);
      GL11.glPushMatrix();
      int button_rgb;
      if (this.mod.isEnabled()) {
         button_rgb = Theme.getTextPrimaryColor().getRGB();
      } else if (this.mod.canBeEnabled()) {
         button_rgb = Theme.getTextSecondaryColor().getRGB();
      } else {
         button_rgb = Theme.getDisabledColor().getRGB();
      }
   ...
   public void mouseDown(int x, int y, int b) {
      if (mod.canBeEnabled()) {
         if (this.ii(x, y) && b == 0) {
            this.mod.toggle(ModuleTransitionSource.GUI);
         }
      }
